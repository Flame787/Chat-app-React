...
export default function Giphy({ onGifSelect }) {
  ...
  const [gf, setGf] = useState(null); // added for fetching giphy-key via Netlify serverless functions

  // NEW - fetching giphy-key via Netlify serverless functions:
  useEffect(() => {
    const fetchKey = async () => {
      try {
        ...
        if (data?.giphyKey) {
          const giphyClient = new GiphyFetch(data.giphyKey);
          setGf(giphyClient);
        } else {
          console.error("GIPHY_KEY not found.");
          setError("Unable to initialize Giphy API.");
        }
      } catch (err) {
        console.error("Error while fetching Giphy key:", err);
        setError("Failed to fetch Giphy key.");
      }
    };
  ...
  return (
    <div className="search-gifs">
      ...
      {gifs.length === 0 && !loading && (
        <p className="no-gifs-found">No GIFs found.</p>
      )}
